...
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (154)
...
any(is.na(time)) # FALSE (no NA's)
cancer_data = cancer_data[, time > 0 & !is.na(time), ]
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (153)
...
# Save Omics data ----
## mRNA ----
mrna = cancer_data[,,'PAAD_RNASeq2GeneNorm-20160128']
...
clin_tbl = colData(cancer_data)[, clinical_vars] %>%
  ...
  dplyr::select(-c('days_to_death', 'days_to_last_followup',
    ...
    'number_pack_years_smoked', 'year_of_tobacco_smoking_onset', # many NAs
