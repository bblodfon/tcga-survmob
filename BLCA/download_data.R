...
# muscle invasive urothelial carcinoma => 409 patients
...
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (409)
...
any(is.na(time)) # TRUE (some NA's, there is also a negative number)
cancer_data = cancer_data[, time > 0 & !is.na(time), ]
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (406)
...
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (330)
...
# Save Omics data ----
## mRNA ----
mrna = cancer_data[,,'BLCA_RNASeq2GeneNorm-20160128']
mrna_mat = t(assay(mrna))

mrna_mat[1:5,1:5] # count data
print(dim(mrna_mat))
readr::write_csv(x = as_tibble(mrna_mat), file = 'BLCA/data/mRNA.csv')

## CNA ----
cna = cancer_data[,,'BLCA_CNASNP-20160128']
...
print(dim(cna_mat))
readr::write_csv(x = as_tibble(cna_mat), file = 'BLCA/data/CNA.csv')

## miRNA ----
mirna = cancer_data[,,'BLCA_miRNASeqGene-20160128']
mirna_mat = t(assay(mirna))

mirna_mat[1:5,1:5] # counts
print(dim(mirna_mat))
readr::write_csv(x = as_tibble(mirna_mat), file = 'BLCA/data/miRNA.csv')

## Methylation ----
meth = cancer_data[,,'BLCA_Methylation-20160128']
...
print(dim(methyl_mat))
readr::write_csv(x = as_tibble(methyl_mat), file = 'BLCA/data/methyl.csv')

## RPPA ----
rppa = cancer_data[,,'BLCA_RPPAArray-20160128']
rppa_mat = t(assay(rppa))

rppa_mat[1:5,1:5]
print(dim(rppa_mat))
readr::write_csv(x = as_tibble(rppa_mat), file = 'BLCA/data/RPPA.csv')

## Clinical ----
# Level 4 clinical & pathological data
clinical_vars = TCGAutils::getClinicalNames(diseaseCode = disease_code)
length(clinical_vars) # 18
...
clin_tbl = colData(cancer_data)[, clinical_vars] %>%
  ...
  dplyr::select(-c('days_to_death', 'days_to_last_followup',
    ...
    'number_pack_years_smoked', 'karnofsky_performance_score', # many NAs
...
readr::write_csv(x = clin_tbl, file = 'BLCA/data/clinical.csv')
