...
#' => Need quite a few memory to run this script
library(survmob)
library(tidyverse)
library(rstanarm)
library(ggdist)
...
p +
  ...
  # restrict values
  xlim(c(-0.3, 0.3)) +
  # add arrows to show that distribution is more to the left
  annotate(
    geom = 'segment',
    x = -0.3, xend = -0.3,
...
p +
  geom_vline(xintercept = 0, linetype = 'dashed', color = '#E41A1C') +
  annotate(geom = 'text', x = 0.007, y = 6, label = 'Kaplan-Meier', color = 'black',
    angle = 90) +
  # restrict values
  xlim(c(-0.03, 0.03)) +
...
ridgeline_plot(post_draws = omics_postdiff_draws_uno[[msr_id]],
  ...
  rope_stats = rs, add_within = T, x_within = -0.0075)
ggsave(file = paste0(res_path_uno, '/omics_ranking_' , msr_id, '.png'),
  units = 'in', width = 7, height = 5, dpi = 300)
...
for(msr_id in names(res)) {
  if (msr_id == 'harrell_c') { # Harrell's C-index
    ...
    message('# Measure: ', msr_id)
    rs = rope_stats(post_draws = res[[msr_id]], size = 0.05)
    write_csv(rs, file = paste0(res_path_cmp, '/ROPE_stats_' ,
      0.05, '_', msr_id, '.csv'))
    ...
    ridgeline_plot(post_draws = res[[msr_id]],
      ...
      subtitle = 'Uno\'s C-index vs RCLL',
      x_axis_label = 'Difference in IBS (ERV)',
      ROPE_area = TRUE, draw_arrow = FALSE, size = 0.05, pal = 'viridis',
      rope_stats = rs, add_right = T, add_left = T,
    ...
    ridgeline_plot(post_draws = res[[msr_id]],
      ...
      x_axis_label = 'Difference in RCLL (ERV)',
      ROPE_area = TRUE, draw_arrow = TRUE, size = size, pal = 'viridis',
      rope_stats = rs, add_within = T, x_within = -0.01) +
      xlim(c(-0.03, 0.03)) +
