...
#'
#' Execute command: `Rscript LUAD/preprocess.R > LUAD/data/preprocess.log`
library(tidyverse)
library(survmob)
library(mlr3verse)
library(mlr3proba)
...
clin_tbl2 = clin_tbl %>%
  ...
  mutate(pathologic_stage = factor(case_when(
    pathologic_stage == 'stage i' ~ 'stageI',
    ...
    pathologic_stage == 'stage iiia' ~ 'stageIII',
    pathologic_stage == 'stage iiib' ~ 'stageIII',
...
po_scale = po('scale', affect_columns = selector_name(feature_names =
    c('age', 'number_pack_years_smoked'))
)
...
print(paste0('Censoring rate: ',
  floor(100 * sum(status==0)/length(status)), '%')) # 59%
