...
#' => Save data to files
#'
#' Execute command: `Rscript LUAD/download_data.R` (not via Rstudio!)
...
my_assays = c('*RPPA*', '*Methylation_methyl450*', '*miRNA*', '*RNASeq2GeneNorm*',
  '*CNASNP*')
...
print(head(sampleTypes)) # Primary Solid Tumor = '01'
...
cna = cancer_data[,,'LUAD_CNASNP-20160128']
...
# Lung adenocarcinoma => 325 patients
...
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (325)
...
any(is.na(time)) # TRUE (some NA's)
cancer_data = cancer_data[, time > 0 & !is.na(time), ]
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (315)
...
# keep the 4 omics which have the largest amount of patients with all modalities
# mRNA, miRNA, CNA, Methyl
assays_to_keep =
  powcs %>%
  arrange(desc(intersect_count)) %>%
  filter(n_omics == 4) %>%
...
print(paste0('#Patients: ', nrow(colData(cancer_data)))) # number of patients (259)
...
# Save Omics data ----
## mRNA ----
mrna = cancer_data[,,'LUAD_RNASeq2GeneNorm-20160128']
mrna_mat = t(assay(mrna))

mrna_mat[1:5,1:5] # count data
print(dim(mrna_mat))
readr::write_csv(x = as_tibble(mrna_mat), file = paste0(disease_code, '/data/mRNA2.csv'))
...
print(dim(cna_mat))
readr::write_csv(x = as_tibble(cna_mat), file = paste0(disease_code, '/data/CNA2.csv'))

## miRNA ----
mirna = cancer_data[,,'LUAD_miRNASeqGene-20160128']
mirna_mat = t(assay(mirna))

mirna_mat[1:5,1:5] # counts
print(dim(mirna_mat))
readr::write_csv(x = as_tibble(mirna_mat), file = paste0(disease_code, '/data/miRNA2.csv'))

## Methylation ----
meth = cancer_data[,,'LUAD_Methylation_methyl450-20160128']
...
print(dim(methyl_mat))
readr::write_csv(x = as_tibble(methyl_mat), file = paste0(disease_code, '/data/methyl2.csv'))

## Clinical ----
# Level 4 clinical & pathological data
clinical_vars = TCGAutils::getClinicalNames(diseaseCode = disease_code)
length(clinical_vars) # 20
...
clin_tbl = colData(cancer_data)[, clinical_vars] %>%
  ...
  dplyr::select(-c('days_to_death', 'days_to_last_followup',
    'days_to_last_known_alive', # all NAs
    'tumor_tissue_site', 'histological_type', # same value for all patients
    'date_of_initial_pathologic_diagnosis',
    'year_of_tobacco_smoking_onset', 'karnofsky_performance_score', # many NAs
...
readr::write_csv(x = clin_tbl, file = paste0(disease_code, '/data/clinical2.csv'))
